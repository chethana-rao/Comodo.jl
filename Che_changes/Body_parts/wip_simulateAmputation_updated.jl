...
# Cutting the model from the top
...
FF[indSkin] = Fn
VV[indSkin] = Vn
#cutting the plane for the tibia 
...
#boundary edges extracting from the lower part 
...
Fs = FF[1]
Vs = VV[1]
#boundary edges for the bottom part of the skin
Eb = boundaryedges(Fs)
#boundary edges extrating the bottom part of the skin 
...
#smootheing operation
#mean of the bottom skin vertices
...
# FF[1] = Fs[logicSmooth]
VV[1] = Vs
#obtaing the vertext normal from the bottom skin and the tagents. cross product
...
for i in 1:1:length(fileName_set)
    M = GeometryBasics.Mesh(VV[i],FF[i])
    M1 = push!(M1,M)
...
#hp2 = poly!(ax1,GeometryBasics.Mesh(Vn,Fn), color=:white, shading = FastShading, transparency=false,strokecolor=:black,strokewidth=0.25)
