...
#sphere 
r3 = r2/2
Fs,Vs = geosphere(3,r3)
Vs =[GeometryBasics.Point{3, Float64}(v[1]-(r2/2)*0.99,v[2],v[3]+height/2) for v ∈ Vs]
D3 = edgelengths(Fs,Vs) 

#visualization
Fw,Vw,Cw = joingeom(Fhc,Vhc,Fic,Vic,Fs,Vs)
...
hp4 = poly!(ax2,GeometryBasics.Mesh(Vs,Fs), strokewidth=1,color=:white, strokecolor=:black, shading = FastShading, transparency=false,colormap=Makie.Categorical(Makie.Reverse(:Spectral)))
...
v_region2 = Point{3,Float64}(r3+((r2-r3)/2), 0.0, height/2)
v_region3 = Point{3,Float64}(0.0, 0.0, height/2)
vol1 = mean(D1)^3 / (6.0*sqrt(2.0)) #volume of ideal tetrahedron
vol2 = mean(D2)^3 / (6.0*sqrt(2.0))
vol3 = mean(D3)^3 / (6.0*sqrt(2.0))
region_vol = [vol1,vol2,vol3]
E,V,CE,Fb,Cb= tetgenmesh(Fw,Vw; facetmarkerlist=Cw, V_regions=[v_region1,v_region2,v_region3],region_vol=region_vol, stringOpt)
...
hp2 = poly!(ax2,M, color=CE_Vs, shading = FastShading, transparency=false,strokecolor=:black,strokewidth=strokewidth, overdraw=false,colorrange = (1,3),colormap=cmap)
...
#  fileName = comododir()*"/assets/img/TetGen_cylinder_example_02.mp4"
